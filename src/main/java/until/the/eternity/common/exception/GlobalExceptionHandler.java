...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleCustomException(CustomException ex) {
        ...
        return new ResponseEntity<>(ErrorResponse.of(code.getCode(), code.getMessage()), HttpStatus.valueOf(Integer.parseInt(code.getCode())));
    }
    ...
    public ResponseEntity<ErrorResponse> handleMethodNotAllowed(HttpRequestMethodNotSupportedException ex) {
        ...
        return new ResponseEntity<>(ErrorResponse.of("405", "허용되지 않은 HTTP 메서드입니다."), HttpStatus.METHOD_NOT_ALLOWED);
    }

    @ExceptionHandler(Exception.class)
    public ResponseEntity<ErrorResponse> handleException(Exception ex) {
        log.error("Unexpected Error: ", ex);
        return new ResponseEntity<>(ErrorResponse.of("500", "예상치 못한 오류가 발생했습니다."), HttpStatus.INTERNAL_SERVER_ERROR);
    }
}
