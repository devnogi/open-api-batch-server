...
@Entity
@Table(
        name = "subcategory_weekly_statistics",
        indexes = {
            @Index(
                    name = "idx_subcategory_weekly_statistics_category_year_week",
                    columnList = "item_sub_category, year, week_number")
        },
        uniqueConstraints = {
            @UniqueConstraint(
                    name = "uk_subcategory_weekly_statistics_category_year_week",
                    columnNames = {"item_sub_category", "year", "week_number"})
        })
@Getter
@NoArgsConstructor(access = AccessLevel.PROTECTED)
@AllArgsConstructor
@Builder
@Schema(description = "서브카테고리별 주간 통계")
public class SubcategoryWeeklyStatistics {
    ...
    @Schema(description = "거래 총량 (총 거래 금액)", example = "350000000")
    private Long totalVolume;

    @Column(name = "total_quantity", nullable = false)
    @Schema(description = "거래 수량 (itemCount 합계)", example = "10500")
